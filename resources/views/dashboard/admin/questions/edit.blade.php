            ...
            <div class="row align-items-start">
                <div class="col-md-8">
                    <div class="page-separator">
                        <div class="page-separator__text">Question Details</div>
                    </div>
                    <div class="card card-body">
                        <form id="create-test-form" method="post" action="{{ route('questions.update',$question) }}"
                            enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <!-- Your main form content here -->
                            <input id="course_id" value="{{$question->course_id}}" type="text" hidden name="course_id">
                            ...
                            <div class="form-group">
                                ...
                                <select name="question_type" id="question_type" class="form-control custom-select ">
                                    <option @if ($question->question_type=="multiple")  selected @endif value="multiple">Multiple Choice</option>
                                    <option @if ($question->question_type=="essay")  selected @endif value="essay">Essay</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group" id="select01-group">
                                ...
                                <select id="select01" data-toggle="select" name="question_options[]" data-multiple="true" multiple="" class="form-control select2-hidden-accessible" tabindex="-1" aria-hidden="true" data-select2-id="select01">
                                    <!-- Options will be dynamically added here -->
                                   @if ($question->options)
                                   @foreach ($question->options as $key => $option )
                                   <option selected="" data-select2-id="{{$key}}">{{$option}}</option>
                                   @endforeach
                                   @endif
                                {{-- <option selected="" data-select2-id="6">Option 3</option>
                                <option selected="" data-select2-id="9">Option 1</option> --}}
                            </select>
                            </div>
                            ...
                            <div class="form-group" id="textarea-input-group">
                                ...
                                <textarea required class="form-control" name="answer" id="textarea-input">{{$question->answer}}</textarea>
                            </div>

                            <div class="form-group" id="text-input-group">
                                <label class="form-label">Completion Points</label>
                                <input type="number" name="points" value="{{$question->points}}" class="form-control" id="text-input" value="10">
                            </div>
